...
def plot_properties(system, x_values, y_values_1, y_values_2):
    ...
    fig.add_trace(go.Scatter(x=x_values, y=y_values_1, mode='lines', name=y1_label, line=dict(color=color1, width=2, dash='solid')))
    fig.add_trace(go.Scatter(x=x_values, y=y_values_2, mode='lines', name=y2_label, line=dict(color=color2, width=2, dash = 'solid')))
...
def plot_capillary_pressure(system, S, pc):
    ...
    if len(S) > 0:
        fig.add_trace(go.Scatter(x=S, y=pc, mode='lines', name='Capillary Pressure (pc)', line=dict(color='dimgray', width=2, dash = 'solid')))
